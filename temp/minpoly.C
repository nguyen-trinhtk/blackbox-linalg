...
int main (int argc, char **argv)
{
    ...
    for (int a = 0; a < argc; ++a)
        std::cerr << "argv[" << a << "] = " << argv[a] << std::endl;

    if (argc < 2) {
        std::cerr << "Usage: minpoly <matrix-file-in-SMS-format> [<p>]" << std::endl;
        return -1;
    }
    ...
    LinBox::Timer chrono;   // timer available for both cases
    Method::Auto M;

    // Case 1: integer matrix (no modulus provided)
    if (argc != 3) {
        ...
        if (B.rowdim() == B.coldim()) {
            std::clog << "B is " << B.rowdim() << " by " << B.coldim() << std::endl;
            ...
            if (B.rowdim() > B.coldim()) {
                ...
                std::clog << "(B^T . B) is " << A.rowdim() << " by " << A.coldim() << std::endl;
                chrono.start();
                PAR_BLOCK {
                    minpoly(m_A, A, M);
                }
                ...
                std::clog << "(B . B^T) is " << A.rowdim() << " by " << A.coldim() << std::endl;
                chrono.start();
                PAR_BLOCK {
                    minpoly(m_A, A, M);
                }
        ...
        std::cout << "\n⏱  Time elapsed: " << chrono << std::endl;
        std::cout << "Minimal Polynomial: ";
        printPolynomial(std::cout, ZZ, m_A) << std::endl;
    }

    // Case 2: matrix over GF(p)
    else {
        const long p = std::atol(argv[2]);
        Givaro::Modular<int> F(p);
        SparseMatrix<Givaro::Modular<int>> B(F);
        DensePolynomial<Givaro::Modular<int>> m_B(F);

        B.read(input);
        std::cout << "B is " << B.rowdim() << " by " << B.coldim() << " over GF(" << p << ")" << std::endl;
